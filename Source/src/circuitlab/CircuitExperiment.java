...
public class CircuitExperiment extends Experiment
{
	 ...
	 */
	public static final transient String QUERYABLE_SIZE = "Queryable size";
	...
	/**
	 * The parameter "graph generation time"
	 */
	public static final transient String GRAPH_GEN_TIME = "Graph generation time";
	...
	{
		super();
		describe(QUERYABLE_SIZE, "The size (in bytes) of the queryable resulting from the evaluation of the circuit");
		...
		describe(GRAPH_GEN_TIME, "The time (in ms) to generate the designation graph");
	...
	public void execute() throws ExperimentException, InterruptedException 
	{
		...
		long graph_end_time = System.currentTimeMillis();
		...
		if (track)
		{
			write(QUERYABLE_SIZE, q_size);
			write(GRAPH_SIZE, q_size_squashed);
			write(TOTAL_MEM_SQUASHED, f_size - q_size + q_size_squashed);
			write(GRAPH_GEN_TIME, graph_end_time - graph_start_time);
		}
		write(TOTAL_MEM, f_size);
	}
}